...
import type { GatewayModelOption } from '../api/agent';
...
export function AgentParamSelector({ models, recommended = {}, reasons = {}, onParamsChange, keyPrefix = '' }: Props) {
  ...
  const selectStyle: React.CSSProperties = {
    ...
    fontSize: 14, background: 'white', color: 'var(--color-ink)', outline: 'none',
  };

  return (
    <div
      style={{
        marginBottom: 14, padding: '12px 14px', background: 'var(--color-canvas)',
        borderRadius: 10, border: '1px solid var(--color-border)', fontSize: 14,
      }}
    >
      <div style={{ fontSize: 13, color: 'var(--color-muted)', marginBottom: 8 }}>
        {keyPrefix === 'image' ? '图片生成参数' : keyPrefix === 'video' ? '视频生成参数' : '生成参数'}
        （已按推荐选择，可直接确认或修改）
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ color: 'var(--color-muted)', width: 68, flexShrink: 0 }}>模型</span>
          <select value={model} onChange={(e) => setModel(e.target.value)} style={selectStyle}>
            {models.map((m) => (
              <option key={m.value} value={m.value}>{m.label || m.value}</option>
            ))}
          </select>
          {reasons[keyPrefix + 'model'] && <span style={{ color: 'var(--color-primary)', fontSize: 13 }}>{reasons[keyPrefix + 'model']}</span>}
        </div>
        {visibleMeta.map((meta) => (
          <div key={meta.key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: 'var(--color-muted)', width: 68, flexShrink: 0 }}>{meta.label}</span>
            <select
            ...
            {reasons[keyPrefix + meta.key] && (
              <span style={{ color: 'var(--color-primary)', fontSize: 13 }}>推荐：{reasons[keyPrefix + meta.key]}</span>
            )}
          </div>
        ))}
      </div>
