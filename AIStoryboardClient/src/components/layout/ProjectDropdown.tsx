 ...
 */
export function ProjectDropdown({ open, onClose, popupRight }: {
            ...
            projects.map((p) => (
              ...
              >
                <button
                  ...
                  style={{
                    flex: 1, padding: '8px 6px 8px 14px', border: 'none', background: 'none',
                ...
                <div style={{ display: 'flex', alignItems: 'center', paddingRight: 4, flexShrink: 0 }}>
                  <button
                    ...
                    title="重命名"
                    style={{ width: 28, height: 28, border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 'var(--rounded-sm)', color: 'var(--color-muted)' }}
                  >✏️</button>
                  <button
                    onClick={(e) => { e.stopPropagation(); setDeleteTarget(p); }}
                    disabled={projects.length <= 1}
                    title={projects.length <= 1 ? '默认项目不可删除' : '删除'}
                    style={{ width: 28, height: 28, border: 'none', background: 'none', cursor: projects.length <= 1 ? 'not-allowed' : 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 'var(--rounded-sm)', color: 'var(--color-muted)', opacity: projects.length <= 1 ? 0.35 : 1 }}
                  >🗑️</button>
                </div>
              </div>
            ))
