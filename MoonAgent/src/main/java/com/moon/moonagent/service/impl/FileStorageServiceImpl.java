...
public class FileStorageServiceImpl implements FileStorageService {
    ...
    /** 上传根目录（相对进程 cwd；monorepo 本地开发指向主后端 uploads 实现文件共享，生产可配共享存储路径） */
    @Value("${ai.upload-root:uploads}")
    private String uploadRoot;

    private Path imagesDir() { return Paths.get(uploadRoot, "images"); }
    private Path videosDir() { return Paths.get(uploadRoot, "videos"); }
    private Path audiosDir() { return Paths.get(uploadRoot, "audios"); }
    ...
    public String saveImage(String sourceUrl) {
        return downloadAndSave(sourceUrl, imagesDir(), "/api/files/images/", "image/png");
    }
        ...
        try {
            ...
            switch (t) {
                case "video" -> {
                    ...
                    targetDir = videosDir(); urlPrefix = "/api/files/videos/"; allowedExts = ALLOWED_VIDEO_EXTENSIONS;
                }
                case "audio" -> {
                    if (contentType == null || !contentType.startsWith("audio/")) {
                        throw new BusinessException(40001, "仅支持上传音频文件");
                    }
                    targetDir = audiosDir(); urlPrefix = "/api/files/audios/"; allowedExts = ALLOWED_AUDIO_EXTENSIONS;
                }
                default -> {
                    if (contentType == null || !contentType.startsWith("image/")) {
                        throw new BusinessException(40001, "仅支持上传图片文件");
                    }
                    targetDir = imagesDir(); urlPrefix = "/api/files/images/"; allowedExts = ALLOWED_IMAGE_EXTENSIONS;
                }
            }
    ...
    public String saveVideo(String sourceUrl) {
        ...
        if (sourceUrl.startsWith("http://") || sourceUrl.startsWith("https://")) {
            return downloadAndSave(sourceUrl, videosDir(), "/api/files/videos/", "video/mp4");
        }
    ...
    public Path resolveImage(String filename) {
        return safeResolve(imagesDir(), filename);
    }

    @Override
    public Path resolveVideo(String filename) {
        return safeResolve(videosDir(), filename);
    }

    @Override
    public Path resolveAudio(String filename) {
        return safeResolve(audiosDir(), filename);
    }
