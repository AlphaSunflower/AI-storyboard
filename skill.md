...
\- \*\*Already have `FIRECRAWL\_API\_KEY`\*\* — skip credential setup; pick
...
using `FIRECRAWL\_API\_KEY` from the project's `.env` or runtime config
...
If you already have a valid `FIRECRAWL\_API\_KEY`, skip this path.
...
SESSION\_ID=$(openssl rand -hex 32)

CODE\_VERIFIER=$(openssl rand -base64 32 | tr '+/' '-\_' | tr -d '=\\n' | head -c 43)

CODE\_CHALLENGE=$(printf '%s' "$CODE\_VERIFIER" | openssl dgst -sha256 -binary | openssl base64 -A | tr '+/' '-\_' | tr -d '=')
...
https://www.firecrawl.dev/cli-auth?code\_challenge=$CODE\_CHALLENGE\&source=coding-agent#session\_id=$SESSION\_ID
...
{"session\_id": "$SESSION\_ID", "code\_verifier": "$CODE\_VERIFIER"}
...
echo "FIRECRAWL\_API\_KEY=fc-..." >> .env
...
&#x20; `FIRECRAWL\_API\_KEY=fc-...` in your environment or pass it directly
...
\*\*Auth header:\*\* `Authorization: Bearer fc-YOUR\_API\_KEY`
...
FIRECRAWL\_API\_KEY=fc-7988598fd92441d3be49974d9d305124
